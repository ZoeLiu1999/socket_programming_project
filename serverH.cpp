...
std::map<std::string, int> HistoryLib;
int result;
...
// Checkout books according to the rule
void checkValue(const char* key, const char* status) {
    std::string keyString(key);
    auto it = HistoryLib.find(keyString);
    if (it == HistoryLib.end()) {
        result = -1;
    } else {
         int value = it->second;
    if (value == 0) {
...
// UDP socket initialization (copied from Beej's-Guide: listener.c)
 ...
 printf("Server H is up and running using UDP on port 43026.\n");
